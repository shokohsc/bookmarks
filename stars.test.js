...
describe("getStars", () => {
  it("throws when no username provided", async () => {
    await expect(getStars()).rejects.toThrow(
      "Usage: node stars.js <github-username>",
    );
  });

  it("throws when GH_TOKEN is not set", async () => {
    await expect(getStars("testuser")).rejects.toThrow("Please set GH_TOKEN.");
  });
  ...
  it("throws error on non-ok response", async () => {
    ...
    process.env.GH_TOKEN = "token123";
    await expect(getStars("testuser", mockFetch)).rejects.toThrow(
      "403 Forbidden",
    );
  });
